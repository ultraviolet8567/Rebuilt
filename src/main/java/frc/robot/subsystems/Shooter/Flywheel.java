...
public class Flywheel extends SubsystemBase {
    ...
    public Flywheel() {
		...
		followerMotorConfig.encoder.velocityConversionFactor(1.0 / ShooterConstants.kFlywheelReduction);
        followerMotorConfig.smartCurrentLimit(80);
        followerMotorConfig.idleMode(IdleMode.kCoast);
        followerMotor.configure(leadMotorConfig,ResetMode.kResetSafeParameters, PersistMode.kPersistParameters);

        followerMotorConfig.follow(CAN.kFlywheelLeadPort);

        pidController = new PIDController(ShooterConstants.kFlywheelP, ShooterConstants.kFlywheelI, ShooterConstants.kFlywheelD);
    }
